...
export default function FeaturedProjects() {
  const projects: Project[] = [
    ...
    },
    {
      ...
      badges: ["Industry", "Client Delivered"],
      links: {
        primary: {
          url: "https://wesleyeze.com/papers/vvc_paper.pdf",
          label: "View Project",
                ...
                {project.badges && project.badges.length > 0 && (
                  <div className="flex gap-1.5">
                    {project.badges.map((badge, badgeIndex) => (
                      <span
                        key={badgeIndex}
                        className="px-2 py-0.5 text-xs bg-[#695336]/15 text-[#695336] dark:bg-[#695336]/25 dark:text-[#d4c4b0] rounded-full font-medium"
                      >
                        {badge}
                      </span>
                    ))}
                  ...
                  <div className="space-y-2">
                    ...
                    {project.links.primary && (
                      <a
                        ...
                        className="inline-flex items-center gap-1.5 text-sm text-foreground hover:text-foreground font-semibold transition-colors duration-200"
                      >
                        {project.links.primary.icon === 'github' && (
                        ...
                        {project.links.primary.icon === 'youtube' && (
                          <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                        ...
                        {project.links.primary.icon === 'paper' && (
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                          </svg>
                        )}
                        <span>{project.links.primary.label}</span>
                      </a>
                    )}

                    {/* Secondary CTAs */}
                    {project.links.secondary && project.links.secondary.length > 0 && (
                      <div className="flex flex-wrap gap-3">
                        {project.links.secondary.map((link, linkIndex) => (
                          <a
                            key={linkIndex}
                            href={link.url}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="inline-flex items-center gap-1.5 text-xs text-foreground/70 hover:text-foreground/90 transition-colors duration-200"
                          >
                            {link.icon === 'github' && (
